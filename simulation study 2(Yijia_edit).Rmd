...
for(i in 1:M){
  ...
  fit.gee<- gee(y~x, id= id, family = "poisson", data = df)
  
  # iii & iv
  beta.hat <- fit.gee$coefficients
  vcov.hat = fit.gee$robust.variance
  beta_m[[i]] = beta.hat 
  vcov_m[[i]] = vcov.hat
  
  # v
  T = 1/(vcov.hat[2,2])*(beta.hat[2]-1)^2
  m.T = c(m.T, T)
}
```
...
```{r}
 # vi
(alpha_hat<-mean(m.T >= qchisq(0.95, 1)))

# vii
(beta.hat.bar = Reduce('+', beta_m)/M)
(asymp.var = Reduce('+', vcov_m)/M)

emp<- list()
for (i in 1:M) {
  bias=beta_m[[i]]-beta.hat.bar
  bias.m<- matrix(c(bias[1],bias[2]), nrow=2,ncol=1)
  emp[[i]]<- bias.m %*% t(bias.m)
}
